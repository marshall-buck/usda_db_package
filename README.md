...
Every nutrient id in the shipped data is in that table.
...
- 7,006 foods, USDA SR Legacy.
- 93 distinct nutrients across the set; 1 to 81 per food. Calories (1008) is
  present for every food.
- All amounts are per 100 g.
- Two assets, `<hash>_foods_db.json` and `<hash>_autocomplete_hash.json`, plus
  `file_manifest.txt`, which holds the hash their names are prefixed with.

`init()` parses both files into in-memory maps, which stay resident for the
life of the instance so queries never touch the filesystem. The JSON strings
they were read from are dropped once parsing finishes; `dispose()` drops the
maps as well.
...
data instead of reading the bundle. App code should never pass one — the
default reads the packaged assets, and `FileService` is exported only so that
parameter's type can be named from outside the package.
